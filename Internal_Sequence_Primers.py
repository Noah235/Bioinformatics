...
def run_pipeline():
    ...
    with open(output_csv, "w", newline="") as outcsv:
        ...
        writer.writerow([
            "Gene Name",
            "Forward Primer (internal)", "Fwd Tm", "Fwd GC%",
            "Reverse Primer (internal)", "Rev Tm", "Rev GC%",
        ...
        for gene in gene_list:
            ...
            # Internal (full gene) sequence for primer design
            gene_seq = chrom_seq[start:end]
            if strand == -1:
                gene_seq = gene_seq.reverse_complement()

            primers = primer3.bindings.design_primers(
                {'SEQUENCE_ID': gene_name + '_internal', 'SEQUENCE_TEMPLATE': str(gene_seq)},
                DEFAULT_SETTINGS
            )

            fwd_primer = primers.get('PRIMER_LEFT_0_SEQUENCE')
            rev_primer = primers.get('PRIMER_RIGHT_0_SEQUENCE')
            product_len = primers.get('PRIMER_PRODUCT_SIZE', 'N/A')
...
# --- GUI setup ---
root = tk.Tk()
root.title("PCR Primer Design – Internal Gene Primers")
